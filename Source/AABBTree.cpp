...
{
	unsigned nodeIndex = _objectNodeIndexMap[go];
	updateLeaf(nodeIndex, go->model->modelBox);
}

std::forward_list<GameObject*> AABBTree::queryOverlaps(const GameObject* go) const
{
	std::forward_list<GameObject*> overlaps;
	std::stack<unsigned> stack;
	AABB testAabb = go->model->modelBox;
